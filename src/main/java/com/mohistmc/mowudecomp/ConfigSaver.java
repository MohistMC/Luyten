...
public class ConfigSaver {
    ...
    private MoWuDecompPreferences luytenPreferences;
    ...
    private void loadConfig() {
        ...
        luytenPreferences = new MoWuDecompPreferences();
        ...
        try {
            ...
            luytenPreferences = loadMoWuDecompPreferences(prefs);
        } catch (Exception e) {
    ...
    private MoWuDecompPreferences loadMoWuDecompPreferences(Preferences prefs) throws Exception {
        MoWuDecompPreferences newMoWuDecompPrefs = new MoWuDecompPreferences();
        for (Field field : MoWuDecompPreferences.class.getDeclaredFields()) {
            if (Modifier.isStatic(field.getModifiers()))
                continue;
            field.setAccessible(true);
            String prefId = field.getName();
            Object defaultVal = field.get(newMoWuDecompPrefs);

            if (field.getType() == String.class) {
                String defaultStr = (String) (defaultVal == null ? "" : defaultVal);
                field.set(newMoWuDecompPrefs, prefs.get(prefId, defaultStr));

            } else if (field.getType() == Boolean.class || field.getType() == boolean.class) {
                Boolean defaultBool = (Boolean) (defaultVal == null ? Boolean.FALSE : defaultVal);
                field.setBoolean(newMoWuDecompPrefs, prefs.getBoolean(prefId, defaultBool));

            } else if (field.getType() == Integer.class || field.getType() == int.class) {
                Integer defaultInt = (Integer) (defaultVal == null ? Integer.valueOf(0) : defaultVal);
                field.setInt(newMoWuDecompPrefs, prefs.getInt(prefId, defaultInt));
            } else if (field.getType().isEnum()) {
                //noinspection unchecked,rawtypes
                field.set(newMoWuDecompPrefs, Enum.valueOf((Class<Enum>) field.getType(), prefs.get(prefId, defaultVal.toString())));
            }
        }
        return newMoWuDecompPrefs;
    }
    ...
    public void saveConfig() {
        ...
        try {
            ...
            saveMoWuDecompPreferences(prefs);
        } catch (Exception e) {
            MoWuDecomp.showExceptionDialog("Exception!", e);
        }
    }
    ...
    private void saveMoWuDecompPreferences(Preferences prefs) throws Exception {
        for (Field field : MoWuDecompPreferences.class.getDeclaredFields()) {
            if (Modifier.isStatic(field.getModifiers()))
                continue;
            field.setAccessible(true);
    ...
    public MoWuDecompPreferences getMoWuDecompPreferences() {
